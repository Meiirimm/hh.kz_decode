...
const getEmployeeApply = async(req, res) => {
    try {
...
const declineEmployee = async(req, res) => {
    try {
        ...
        sendMail(user.email, `Отказ вакансию ${vacancy.name}`, `
            Компания: ${company.name}, к сожалению ваша кондидатура не подходит на вакансию ${vacancy.name}`)
